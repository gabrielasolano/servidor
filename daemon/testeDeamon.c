...
int main (int argc, char **argv)
{
  ...
  inicia_servidor(&sock_servidor, &servidor, porta);
	...
	/* Configuracao inicial para lista de clientes ativos */
	for (i = 0; i < MAXCLIENTS; i++)
	{
		zera_struct_cliente(i);
	}

	/* Configuracao inicial : tempo de espera do servidor para novo cliente */
	tempo_espera.tv_sec = 2;
	tempo_espera.tv_usec = 500000;
  ...
  while (1)
  {
    ...
    /* read_fds recebe as conexoes dos clientes */
		if (ativos > 0)
    {
      pedido_cliente = select(max_fd + 1, &read_fds, NULL, NULL, &tempo_espera);
    }
    else
    {
    ...
    if (pedido_cliente == -1)
		...
		if (FD_ISSET(sock_servidor, &read_fds) && (ativos < MAXCLIENTS) && (pedido_cliente > 0))
		{
			...
			else
			{
				...
				clientes_ativos[ativos].buf_enviados = 0;
...
void zera_struct_cliente (int indice_cliente)
{
	...
	clientes_ativos[indice_cliente].buf_enviados = 0;
	memset(clientes_ativos[indice_cliente].caminho, '\0', BUFFERSIZE+1);
		
	//if (clientes_ativos[indice_cliente].fp != NULL)
	//	fclose(clientes_ativos[indice_cliente].fp);
}
...
void responde_cliente (int indice_cliente)
{
		/* Se a quantidade de buffers enviados for 0: envia o cabecalho */
	/* Senao: envia mais um buffer, aumenta o cliente.buf_enviado */
	/* Se chegou no fim do arquivo: seta cliente.enviar para 0 */
	if (clientes_ativos[indice_cliente].buf_enviados == 0)
	{
...
void envia_primeiro_buffer (int indice_cliente)
{
  ...
  //sleep(5);
	int sock_cliente = clientes_ativos[indice_cliente].sock;
      ...
      else
      {
        ...
        else
        {
          ...
          if (existe_pagina(clientes_ativos[indice_cliente].caminho))
          {
						...
						//clientes_ativos[indice_cliente].fp = fopen(clientes_ativos[indice_cliente].caminho, "rb");
						//if (clientes_ativos[indice_cliente].fp == NULL)
						if (fp == NULL)
						{
							perror("fopen()");
...
void envia_buffer (int indice_cliente)
{
  ...
  int sock_cliente;
	...
	/* Verifica a quantidade de buffers enviados e posiciona o ponteiro do arquivo para ler o seguinte */
	
	sock_cliente = clientes_ativos[indice_cliente].sock;
	pular_buf = clientes_ativos[indice_cliente].buf_enviados * sizeof(buf);
	bytes_lidos = fread(buf+pular_buf, 1, sizeof(buf), fp);
	
	if (bytes_lidos > 0)
	{
    envia_cliente(sock_cliente, buf, bytes_lidos);
		clientes_ativos[indice_cliente].buf_enviados++;
	}
	else
	{
		clientes_ativos[indice_cliente].enviar = 0;
